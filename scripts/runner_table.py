...
def main() -> int:
    ...
    for address, raw in candles.items():
        ...
        # A token whose FIRST candle is already above the threshold did not
        # cross within the observable window -- it crossed before the data
        # begins. Measuring a multiple from that point measures the tail of a
        # move that already happened, and on a sample of the largest tokens it
        # produced a median "peak multiple" of 1.09x for tokens that had all
        # reached $2M. The crossing must be observed, not assumed.
        left_censored = crossing == 0
    ...
    # Multiples are only meaningful where the crossing was observed.
    observed = [r for r in rows if not r["left_censored"]]
    censored = [r for r in rows if r["left_censored"]]
    if not observed:
        print(f"\nAll {len(rows)} tokens were already above the threshold on "
              f"their first candle, so no crossing is observable and no multiple "
              f"can be measured. The candle window does not reach back far "
              f"enough for these tokens; fetch smaller-cap pools instead.",
              file=sys.stderr)
        return 1
    ...
    multiples = [r["peak_multiple"] for r in observed]
    realisables = [r["realisable_multiple"] for r in observed
                   if r["realisable_multiple"]]
    ...
    lines = [
        ...
        "",
        f"- **{len(rows)} tokens** reached {money(args.threshold)} or more",
        f"- **{len(observed)}** were observed crossing it (the rest were already "
        f"above it on their first candle, so no multiple can be measured)",
    ...
    for _unused_index, row in enumerate([], start=1):
        if row["left_censored"]:
            ...
            crossed = "before data"
            peak = realisable = days = "n/a"
        else:
            crossed = row["crossed_on"]
            peak = f"{row['peak_multiple']:.1f}x"
            realisable = (f"{row['realisable_multiple']:.1f}x"
                          if row["realisable_multiple"] else "—")
            days = str(row["days_to_ath"])
        lines.append(
            f"| {index} | **{row['token']}** | {crossed} | "
            f"{money(row['ath_cap'])} | {row['ath_on']} | "
            f"{peak} | {realisable} | {days} | "
            f"{money(row['cap_now'])} | "
            f"{'yes' if row['is_runner'] else 'no'} |"
        )
    lines += [
    ...
    print(f"\n  {len(rows)} tokens reached {money(args.threshold)}+ "
          f"({len(observed)} observed crossing, {len(censored)} left-censored); "
